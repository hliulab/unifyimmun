...
def train_tcr(model, train_loader, fold, epoch, epochs):
    model.encoder_P.load_state_dict(torch.load('../HLA_1/encoder_P_{}.pth'.format(fold)))
    train_time = 0
    model.train()
...
for fold in range(1, 6):
    ...
    train_data_tcr = pd.read_csv('../data/data_tcr_new/train_fold_{}_balanced.csv'.format(fold))
    ...
    print('TCR Train:')
    path_all_tcr = '../trained model/TCR_1'
    save_path_tcr = '../trained model/TCR_1/model_TCR_fold{}.pkl'.format(fold)
    encoder_path_tcr = '../trained model/TCR_1/encoder_P_{}.pth'.format(fold)
