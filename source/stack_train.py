...
def train_tcr(model, train_loader, fold, epoch, epochs):
    model.encoder_P.load_state_dict(torch.load('../trained model/HLA_stack/encoder_P_{}.pth'.format(fold)))
    train_time = 0
    model.train()
    y_true_list, y_pred_list,attention_list = [], [],[]
    loss_list = []
...
dataset = pd.read_csv('../data/data_HLA_new/dataset.csv')
...
dataset_tcr = pd.read_csv('../data/data_tcr_new/dataset.csv')
...
def Stack_Train(use_tcr_Encoder):
    ...
    for fold in range(1, 11):
        ...
        print('HLA Train:')
        path_all = '../trained model/HLA_stack'
        save_path = '../trained model/HLA_stack/model_head{}_fold{}.pkl'.format(n_heads, fold)
        encoder_path = '../trained model/HLA_stack/encoder_P_{}.pth'.format(fold)
    ...
    for fold in range(1, 11):
        ...
        print('TCR Train:')
        path_all_tcr = '../trained model/TCR_stack'
        save_path_tcr = '../trained model/TCR_stack/model_head{}_fold{}.pkl'.format(n_heads, fold)
        encoder_path_tcr = '../trained model/TCR_stack/encoder_P_{}.pth'.format(fold)
