...
def train_HLA(model, train_loader, fold, epoch, epochs, use_tcr_Encoder=False):
    ...
    if use_tcr_Encoder is True:
        ...
        model.encoder_P.load_state_dict(torch.load('UnifyImmun/TCR_1/encoder_P_{}.pth'.format(fold)))
...
for fold in range(1, 6):
    ...
    train_data = pd.read_csv('UnifyImmun/data/data_HLA_new/train_fold_{}.csv'.format(fold))
    val_data = pd.read_csv('UnifyImmun/data/data_HLA_new/train_fold_{}.csv'.format(fold))
    print('Fold-{} Label: Train = {} | Val = {}'.format(fold, Counter(train_data.label), Counter(val_data.label)))
    print('HLA Train:')
    path_all = 'UnifyImmun/HLA_2'
    save_path = 'UnifyImmun/HLA_2/model_HLA_fold{}.pkl'.format( fold)
    encoder_path = 'UnifyImmun/HLA_2/encoder_P_{}.pth'.format(fold)
